...
const TopBar = ({
  ...
  const [buyUrl, setBuyUrl] = useState('https://app.pangolin.exchange/#/swap?inputCurrency=0x76076880e1ebbce597e6e15c47386cd34de4930f');
  const [opusBal, setOpusBal] = useState(0)
  ...
  useEffect(() => {
    if ((walletAddr !== "Connect" && walletAddr !== "") && (chainInfo && chainInfo.rpcUrls)) {
      ...
      tokenContract.methods.balanceOf(walletAddr).call(
        ...
        setOpusBal(Number(res) / Math.pow(10, TOKEN_DETAIL.decimals))
      })
    } else {
      setOpusBal(0)
    }
    ...
    <div className="topbar">
      <div className="topbar__icons">
        ...
        <button className="topbar__icon dark:bg-shark-light-500" onClick={() => {window.open("https://bridge.canopus.network/", '_blank')}}>
          <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" class="MuiSvgIcon-root"
        ...
        <button className="topbar__icon dark:bg-shark-light-500">
          <Language onClick={() => {window.open("https://canopus.network/", '_blank')}}/>
        </button>
      </div>
